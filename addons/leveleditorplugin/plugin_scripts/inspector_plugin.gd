...
var data_property = preload("res://addons/leveleditorplugin/plugin_scripts/data_property.gd")
...
func _parse_begin(object):
	...
	add_custom_control(Label.new())
	add_property_editor("Data component" , data_property.new())
...
func create_control_label(t : String , image : Texture2D , background_color : Color) :
	...
	container.get_child(1).text = t
	container.get_child(0).get_child(0).set_texture(image)
